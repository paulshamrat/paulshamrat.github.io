...
The fetcher script constructs a query for papers published in the last 30 days. It extracts the titles, authors, and abstracts, passing them to the **Gemini 2.5 Flash** model with a specific prompt:

> "You are a scientific reporter writing a short Monthly Research Digest for our community. Write a CONCISE, flowing narrative of MAXIMUM 300 words that highlights the most important trends and findings... Focus on the 2-3 most impactful themes."
...
### Automation: GitHub Actions Everyday
...
Every night, the GitHub Action provisions an Ubuntu runner, sets up Python 3.10, and executes the fetcher script. The `GEMINI_API_KEY` is securely injected as a repository secret. If the script detects new papers and generates a new markdown digest in the `_research/` directory, the workflow automatically commits and pushes the file back to the `main` branch. 
